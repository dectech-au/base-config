...
# winetricks corefonts dinput8 directplay input8 nocrashdialog vcrun2015

##### Libraries which breaks shit:
# dotnet40 gdiplus

##### Force Wine to handle keyboard and mouse input:
# wine reg add "HKCU\\Software\\Wine\\X11 Driver" /v "UseTakeFocus" /d "Y" /f && wine reg add "HKCU\\Software\\Wine\\X11 Driver" /v "UseXVidMode" /d "N" /f && wine reg add "HKCU\\Software\\Wine\\X11 Driver" /v "GrabKeyboard" /d "Y" /f && wine reg add "HKCU\\Software\\Wine\\X11 Driver" /v "GrabPointer" /d "Y" /f && wine reg add "HKCU\\Software\\Wine\\X11 Driver" /v "MouseWarpOverride" /d "force" /f
